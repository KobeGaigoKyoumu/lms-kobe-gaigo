...
const PREFECTURES = [
    ...
    { code: '46', name: '資格島県' }, { code: '47', name: '沖縄県' }
].map(p => p.name === '資格島県' ? { ...p, name: '鹿児島県' } : p) // タイポ修正
...
export default function AdminSchoolSearchClient() {
    ...
    const renderSchoolStats = (stats) => {
        if (!stats) return null;

        const { passCount, enrollCount, jlpt } = stats;

        return (
            <div className={styles.statsSection}>
                <h4 className={styles.statsTitle}>進路・合格実績 (過去統計)</h4>
                <div className={styles.statsGrid}>
                    <div className={styles.statsBox}>
                        <span className={styles.statsLabel}>合格者数</span>
                        <span className={styles.statsValue}>{passCount} <small>人</small></span>
                    </div>
                    <div className={styles.statsBox}>
                        <span className={styles.statsLabel}>進学者数</span>
                        <span className={styles.statsValue} style={{ color: '#16a34a' }}>{enrollCount} <small>人</small></span>
                    </div>
                </div>
                {jlpt && jlpt.total > 0 && (
                    <div className={styles.jlptStats}>
                        <div className={styles.jlptHeader}>
                            <span>JLPT N3以上保有率</span>
                            <strong className={styles.overN3Rate}>{jlpt.overN3_rate}%</strong>
                        </div>
                        <div className={styles.jlptProgressTrack}>
                            <div className={styles.jlptProgressFill} style={{ width: `${jlpt.overN3_rate}%` }}></div>
                        </div>
                        <div className={styles.jlptLevelBreakdown}>
                            <span>N1: {jlpt.N1_rate}% ({jlpt.N1}人)</span>
        ...
        <div className={styles.container}>
            ...
            <div className={styles.header}>
                <div className={styles.headerTitleArea}>
                    <h1 className={styles.title}>学校検索（教員用実績統計）</h1>
                    <p className={styles.subtitle}>大学・短大・専門学校の検索に加え、過去の合格・進学者実績とJLPT保有レベル統計を確認できます。</p>
                </div>
            </div>
            ...
            <div className={styles.resultArea}>
                ...
                {!loading && !error && !searched && (
                    <div className={styles.emptyContainer}>
                        ...
                        <p className={styles.emptyText}>学校名や学部・学科名を入力するか、都道府県や学校種別を選択してください。</p>
                    </div>
                )}
                ...
                {!loading && !error && searched && schools.length === 0 && (
                    <div className={styles.emptyContainer}>
                        ...
                        <p className={styles.emptyText}>キーワードのスペルや絞り込み条件（都道府県、学校種別）を変更して再度お試しください。</p>
                    </div>
                )}
