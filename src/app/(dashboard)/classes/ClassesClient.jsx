...
export default function ClassesClient({ adminMember }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch classes and student counts in parallel
                ...
                studentsRes.data.forEach(s => {
                    if (s.class_name) {
                        const trimmed = s.class_name.trim()
                        studentCountByClass[trimmed] = (studentCountByClass[trimmed] || 0) + 1
                    }
                })

                // Process classes
                const currentAcademicYear = 2026
                const processed = (classesRes.data || [])
                    .map(cls => ({
                        ...cls,
                        studentCount: studentCountByClass[cls.name?.trim()] || 0
                    }))
                    .filter(cls => cls.academic_year === currentAcademicYear)

                setAllClasses(processed)
            } catch (err) {
