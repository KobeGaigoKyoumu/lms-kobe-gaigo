...
CREATE TABLE IF NOT EXISTS public.grade_records (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  student_id_text TEXT NOT NULL, -- �w�Дԍ��iExcel�R���j
  student_name TEXT,
  class_name TEXT,
  year_term TEXT, -- �w�����i��: '2025-02'�j
  final_exam_data JSONB, -- ���������ڍ�
  report_card_data JSONB, -- ���ђʒm�\�ڍ�
...
CREATE POLICY "���t�ƊǗ��҂͐��т�ǉ��\" ON public.grade_records
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE id = auth.uid() AND role IN ('teacher', 'admin')
...
CREATE POLICY "���t�ƊǗ��҂͐��т��X�V�\" ON public.grade_records
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE id = auth.uid() AND role IN ('teacher', 'admin')
...
CREATE POLICY "���уf�[�^�͋��t�Ǘ��҂��{���\" ON public.grade_records
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE id = auth.uid() AND role IN ('teacher', 'admin')
